...
int FixedFieldBuffer :: ReadHeader (istream & stream)
{
	...
	if (Initalized) // check header for consistency
	{
...
{
	int result;
	if (!Initalized) return -1; // cannot write unitialized buffer
...
int FixedFieldBuffer :: Pack (const string& field)
{
...
int FixedFieldBuffer :: Unpack (string& field) 
{
	Packing = false;
    ...
    field = Buffer.substr(start, packSize);
